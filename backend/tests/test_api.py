...
async def test_devices(access_token):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"{BASE_URL}/devices/types", json=device_type, headers=headers)
        print(f"Create Device Type: {response.status_code}")
        if response.status_code == 201:
            device_type_id = response.json().get("id")
        elif response.status_code == 400:
            response = await client.get(f"{BASE_URL}/devices/types", headers=headers)
            if response.status_code == 200 and len(response.json()) > 0:
                device_type_id = response.json()[0].get("id")
        
        brand = {"name": f"Dell_{unique_id}"}
        response = await client.post(f"{BASE_URL}/devices/brands", json=brand, headers=headers)
        print(f"Create Brand: {response.status_code}")
        if response.status_code == 201:
            brand_id = response.json().get("id")
        elif response.status_code == 400:
            response = await client.get(f"{BASE_URL}/devices/brands", headers=headers)
            if response.status_code == 200 and len(response.json()) > 0:
        ...
        if device_type_id and brand_id:
            ...
            response = await client.post(f"{BASE_URL}/devices/models", json=model, headers=headers)
            print(f"Create Model: {response.status_code}")
            if response.status_code == 201:
                model_id = response.json().get("id")
            elif response.status_code == 400:
                response = await client.get(f"{BASE_URL}/devices/models", headers=headers)
                if response.status_code == 200 and len(response.json()) > 0:
        ...
        if brand_id and model_id and device_type_id:
            ...
            response = await client.post(f"{BASE_URL}/devices", json=device, headers=headers)
...
async def test_orders(access_token, device_id):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"{BASE_URL}/orders", json=order_data, headers=headers)
        print(f"Create Order: {response.status_code}")
        if response.status_code == 201:
            order_id = response.json().get("id")
            print(f"Order created with ID: {order_id}")
        
        response = await client.get(f"{BASE_URL}/orders", headers=headers)
        print(f"List Orders: {response.status_code} - Count: {len(response.json())}")
        
        response = await client.get(f"{BASE_URL}/orders?status=Pending", headers=headers)
...
async def test_assigns(access_token, order_id):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"{BASE_URL}/assigns", json=assign_data, headers=headers)
        print(f"Create Assign: {response.status_code}")
        if response.status_code == 201:
            assign_id = response.json().get("id")
            print(f"Assign created with ID: {assign_id}")
        
        response = await client.get(f"{BASE_URL}/assigns", headers=headers)
        print(f"List Assigns: {response.status_code} - Count: {len(response.json())}")
        
        if assign_id:
            response = await client.get(f"{BASE_URL}/assigns/{assign_id}", headers=headers)
